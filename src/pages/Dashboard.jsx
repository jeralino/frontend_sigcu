import { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { FaWallet, FaSignOutAlt, FaUser, FaBookOpen, FaUtensils, FaShoppingCart } from "react-icons/fa";

const API_URL = "http://localhost:4000/api";

export default function Dashboard() {
  const navigate = useNavigate();
  const [user, setUser] = useState(null);
  const [facultades, setFacultades] = useState([]);
  const [historial, setHistorial] = useState([]);
  const [saldo, setSaldo] = useState(0);

  // 1. Cargar usuario del LocalStorage
  useEffect(() => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) {
      setUser(JSON.parse(storedUser));
    } else {
      navigate("/login");
    }
  }, [navigate]);

  // 2. Obtener datos
  useEffect(() => {
    if (!user) return;

    const token = localStorage.getItem("token");
    const authHeaders = {
      "Content-Type": "application/json",
      "Authorization": `Bearer ${token}` 
    };

    // A) Cargar Facultades
    fetch(`${API_URL}/facultades`)
      .then(res => {
        if (!res.ok) throw new Error("Error al cargar facultades");
        return res.json();
      })
      .then(data => setFacultades(data))
      .catch(() => setFacultades([])); // Si falla, mostramos vacío sin error rojo

    // B) Cargar Historial (Manejo de error 404)
    fetch(`${API_URL}/reservas/${user.id}`, { headers: authHeaders })
      .then(res => {
        if (res.status === 404) return []; // Si no hay ruta, devuelve array vacío
        if (!res.ok) throw new Error("Error historial");
        return res.json();
      })
      .then(data => setHistorial(Array.isArray(data) ? data : []))
      .catch(() => setHistorial([])); 

    // C) OBTENER SALDO
    const obtenerSaldo = async () => {
      try {
        const res = await fetch(`${API_URL}/usuarios/${user.id}`, { headers: authHeaders });
        
        if (res.ok) {
          const data = await res.json();
          setSaldo(Number(data.saldo) || 0);
        } else {
          console.warn(`No se pudo obtener saldo (Error ${res.status}). Posible problema de permisos.`);
        }
      } catch (error) {
        console.error("Error de conexión");
      }
    };

    obtenerSaldo();

  }, [user]);

  if (!user) return <div className="text-center mt-5 text-white">Cargando...</div>;

  return (
    <div className="d-flex flex-column align-items-center vh-100 text-white"
         style={{ background: "linear-gradient(135deg, #004e92, #000428)", overflowY: "auto" }}>
      
      <header className="w-100 p-4 d-flex justify-content-between align-items-center">
        <h2 className="fw-bold d-flex align-items-center gap-2">
          <FaUser /> Bienvenido, {user.nombre}
        </h2>
        <div className="d-flex align-items-center gap-3">
          <span className="d-flex align-items-center gap-1 fw-bold text-white bg-success px-3 py-1 rounded-pill shadow">
            <FaWallet /> ${saldo.toFixed(2)}
          </span>
          <button
            className="btn btn-outline-light d-flex align-items-center gap-1"
            onClick={() => {
              localStorage.clear();
              navigate("/login");
            }}
          >
            <FaSignOutAlt /> Cerrar sesión
          </button>
        </div>
      </header>

      <main className="container mt-5 d-flex justify-content-center align-items-start flex-wrap gap-4 pb-5">
        
        {/* PANEL FACULTADES */}
        <div className="col-md-5 p-4 rounded-4 shadow-lg text-center"
             style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(10px)" }}>
          <h4 className="d-flex justify-content-center align-items-center gap-2 mb-4">
            <FaBookOpen /> Facultades Disponibles
          </h4>
          <ul className="list-unstyled mt-2">
            {facultades.length === 0 && <li className="text-white opacity-75">No hay facultades disponibles</li>}
            {facultades.map(f => (
              <li
                key={f.id}
                className="p-3 my-2 rounded-3 text-white d-flex align-items-center justify-content-center gap-2"
                style={{ background: "rgba(255,255,255,0.1)", cursor: "pointer" }}
                onClick={() => navigate(`/comedores/${f.id}`)}
              >
                <FaUtensils /> {f.nombre}
              </li>
            ))}
          </ul>
        </div>

        {/* PANEL HISTORIAL */}
        <div className="col-md-5 p-4 rounded-4 shadow-lg text-center"
             style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(10px)" }}>
          <h4 className="d-flex justify-content-center align-items-center gap-2 mb-4">
            <FaShoppingCart /> Historial de Compras
          </h4>
          <ul className="list-unstyled mt-2">
            {historial.length === 0 && <li className="text-white opacity-75">No has realizado compras aún</li>}
            {historial.map(r => (
              <li key={r.id} className="p-2 my-2 rounded-3 text-white bg-dark bg-opacity-50">
                {r.plato_nombre} - {r.estado}
              </li>
            ))}
          </ul>
        </div>

      </main>
      
      <footer className="mt-auto p-3 opacity-50 text-center w-100">
        <small>© 2025 Universidad Técnica de Manabí</small>
      </footer>
    </div>
  );
}